...
int main() {
    ...
    /* เรียกใช้ function เพื่อตรวจสอบว่าเจอ ID หรือไม่ */
...
/* Function ตรวจสอบว่า search_id อยู่ใน array หรือไม่ (เจอคืน 1, ไม่เจอคืน 0) */
int check_id_existence(struct Student students[], int size, int search_id) {
    int i;

    for (i = 0; i < size; i++) {
        if (students[i].id == search_id) {
            return 1; /* เจอแล้ว ออกจาก function ทันที */
        }
    }

    return 0; /* วนหมดแล้วยังไม่เจอ */
}